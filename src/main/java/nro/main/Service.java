...
//            msg.writer().writeInt(pl.vang);
           ...
           ByteArrayInputStream is = new ByteArrayInputStream(FileIO.readFile("res/cache/head"));
            ...
            short lengthHead = dis.readShort();
                   msg.writer().writeShort(lengthHead + 25);
...
//        Util.log("INIT initBillWhis XONG MAP " + Server.gI().maps[idMap].template.name + ", " + IDZONE);
    ...
    public void initSuperBee() {
        int idMap = Util.nextInt(0, 3);
        short xSuperBee = (short)379;
        if(idMap == 0) {
            idMap = 6;
        } else if(idMap == 1) {
            idMap = 10;
        ...
        int IDZONE = Util.nextInt(0, Server.gI().maps[idMap].area.length);
        Boss _SuperBee = new Boss(531, (byte)51, (short)379, (short)456);

        Server.gI().maps[idMap].area[IDZONE].bossMap.add(_SuperBee);
        Server.gI().maps[idMap].area[IDZONE].loadBossNoPet(_SuperBee);
        sendThongBaoServer("BOSS " + _SuperBee.name + " vừa xuất hiện tại " + Server.gI().maps[idMap].template.name);
//        Util.log("INIT SuperBee XONG MAP " + Server.gI().maps[idMap].template.name + ", " + IDZONE);
    }
    //NGOC RONG SAO DEN
