        ...
        if((System.currentTimeMillis() - p._timeDapDo) >= 1000 && p._checkDapDo) {
            ...
            if(p.ItemBag[p._indexUpStar].quantity >= 1 && p.ItemBag[p._indexEpStar].quantity >= 999 && p.vang >= 200000000L) {
                ...
                if(Util.nextInt(0, 100) < perSuccess) {
                    ...
                    if (perSuccess == 49) { perSuccess = 50; }
...
//                    Util.log("perLucky: " + perLucky);
    ...
    public void ItemExchange(Player _player, byte _index) {
        Item _item = _player.ItemBag[_index];
        if(_item == null) {
            return;
        }
        if(!IsItemThanLinh(_item.template.id)) {
            Service.gI().serverMessage(_player.session, "Không phải là đồ thần linh");
            return;
        } else {
            _player._itemUpStar = _item;
            _player._indexUpStar = _index;
            String _info = "Chuyển hóa "+_item.template.name +" thành 1 Item có giá trị";
            if(_player.vang >= 2000000) {
    //            Util.log("STRING INFO ITEM: " + _info);
                ...
                Service.gI().serverMessage(_player.session, "Cần 2tr vàng");
            }
        }
        
    }
public void sendUpSItemExchange(Player _player) {
        Item _itemUp = _player._itemUpStar;
        if(_itemUp == null) {
            return;
        }
        if(!IsItemThanLinh(_itemUp.template.id)) {
            Service.gI().serverMessage(_player.session, "Không Phải Đồ Thần Linh");
            return;
        } else {
            Message m = null;
            if((System.currentTimeMillis() - _player._timeDapDo) >= 1000 && _player._checkDapDo) {
                ...
                if(_itemUp != null && (_player.vang >= 2000000)) {
                    _player.vang -= 2000000;
                   int ListItem[]={15,16,17,381,382,383,384};
                   Item itemadd = ItemSell.getItemNotSell(ListItem[Util.nextInt(0, 7)]);
                   itemadd.quantity = 1;
                   _player.addItemToBag(itemadd);
                   _player.sendAddchatYellow("bạn nhận đc x"+itemadd.quantity +" "+itemadd.template.name);                                     
                    Service.gI().updateVangNgoc(_player);                        
                        Service.gI().updateItemBag(_player);
                        Service.gI().sendUpStarSuccess(_player);
    ...
    private boolean IsItemThanLinh(int id) {
...
//                    System.out.println("ZONE id: " + playerNear.id);
                    if (playerNear.id != p.id && Math.abs(p.x - playerNear.x) <= 200) {
                        playerNear.zone.chat(playerNear, "Em ấy nhìn đươc vã ò");
                    }
                }
            };
        };
        timeEffectChat.schedule(sendEffect, 0, 2000);
